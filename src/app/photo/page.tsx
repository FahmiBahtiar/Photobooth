...
const filterOptions = [
  ...
  { id: "amber", name: "Amber", style: "sepia(80%) hue-rotate(-20deg)" },
  {
    id: "nocturne",
    name: "Night",
    style: "brightness(0.8) contrast(120%) saturate(1.2) hue-rotate(180deg)",
  },
];
...
const PhotoPage = () => {
  ...
  useEffect(() => {
    async function loadVideoDevices() {
      try {
        ...
        // Try to auto-select Fujifilm webcam if available
        const fujifilm = videoInputs.find((device) =>
          device.label.toLowerCase().includes("fujifilm")
        );
        setSelectedDeviceId(
          fujifilm?.deviceId || videoInputs[0]?.deviceId || null
        );
      } catch (err) {
  ...
  const proceedToResults = () => {
    ...
    if (!capturedPhotos.includes(null)) {
      ...
      const validPhotos = capturedPhotos.filter(
        (photo) => photo !== null
      ) as string[];
    ...
    <Layout>
      ...
      <div className="container mx-auto px-3 pt-0 pb-2">
        ...
        <div className="flex items-center mb-1">
          <Link
            href="/"
            className="flex items-center text-gray-700 hover:text-amber-700 transition text-sm"
          >
              ...
              {cameraError ? (
                <div className="text-center p-3 bg-red-50 rounded-lg mb-4 max-w-xl mx-auto">
                  <p className="text-red-600 mb-2 text-sm">
                    Unable to access camera. Please ensure you have granted
                    camera permissions, or use the photo upload feature.
                  </p>
                  <button
                    onClick={() => setActiveTab("upload")}
                  ...
                  <div className="flex-1">
                    ...
                    >
                      ...
                      {videoDevices.length > 1 && (
                        <div className="mb-3 w-full max-w-[600px] mx-auto">
                          <label
                    ...
                    {captureState === CaptureState.CAPTURING && (
                      <div className="text-center w-full max-w-[600px] mx-auto mt-3">
                        <p className="text-gray-600 font-medium text-base">
                          Taking photo {currentPhotoIndex + 1} of 4...
                        </p>
                      </div>
                    )}
                ...
                <p className="text-center text-gray-600 mb-4 text-sm">
                  ...
                  {uploadedPhotos.length > 0 && uploadedPhotos.length < 4 && (
                    <span className="font-medium text-amber-700">
                      {" "}
                      {4 - uploadedPhotos.length} more photos needed.
                    </span>
                  )}
                </p>
